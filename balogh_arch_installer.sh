...
sleep 5
sed 's/fmask=0022/fmask=0137/' /etc/fstab
sed 's/dmask=0022/dmask=0027/' /etc/fstab
cat /etc/fstab
sleep 5
clear
...
sed 's/#hu_HU.UTF-8 UTF-8/hu_HU.UTF-8 UTF-8/' /etc/locale.gen
...
sleep 5
sed 's/HOOKS=(base udev autodetect microcode modconf kms keyboard keymap consolefont block filesystems fsck)/HOOKS=(base systemd autodetect microcode modconf kms keyboard sd-vconsole block sd-encrypt lvm2 filesystems fsck)/' /etc/mkinitcpio.conf
